...
function Topic({ item, setTopicList, topicList, close }) {
                    ...
                    <>
                        ...
                        <Popover.Panel className="w-[294.922px] rounded-xl grid absolute overflow-hidden top-2 right-0 bg-[color:var(--background-primary)] shadow-box z-[1]">
                            ...
                            <button
                                onClick={() => {
                                    setTopicList((x) => {
                                        const filteredList = topicList.filter((x) => x.id !== item.id)
                                        return [...filteredList]
                                ...
                                type='button' className='hover:bg-[color:var(--background-secondary)] transition-colors text-left py-3 px-4 flex items-center gap-3 text-[15px] leading-5 font-bold'>
                                <svg viewBox="0 0 24 24" width={18.75}>
                                    <path
                                        fill='currentColor'
